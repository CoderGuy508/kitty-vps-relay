...
func (service *relayService) handleRelay(response http.ResponseWriter, request *http.Request) {
	...
	gameToken, tokenOK := gameTokenFromRequest(request)
	if !ok || !tokenOK {
	...
	// Browser WebSockets require a selected subprotocol whenever one was
	// requested. The token has already been validated above; acknowledge only
	// its matching protocol. Copy the upgrader per request: mutating the shared
	// Subprotocols slice would leak one tunnel's protocol into another handshake.
	upgrader := service.upgrader
	upgrader.Subprotocols = []string{"kitty-bot-token." + base64.RawURLEncoding.EncodeToString([]byte(gameToken))}
	client, err := upgrader.Upgrade(response, request, nil)
	if err != nil {
		return
	}
	defer client.Close()
	service.relayTunnel(request, client, gameToken)
}

func gameTokenFromRequest(request *http.Request) (string, bool) {
	const prefix = "kitty-bot-token."
	values := strings.Split(request.Header.Get("Sec-WebSocket-Protocol"), ",")
	found := ""
	for _, value := range values {
		...
		if strings.HasPrefix(value, prefix) {
			...
			found = strings.TrimPrefix(value, prefix)
		}
	}
	if found == "" {
		return "", false
	}
	decoded, err := base64.RawURLEncoding.DecodeString(found)
...
func (service *relayService) targetAllowed(rawTarget string) (*url.URL, bool) {
	...
	if err != nil || target.Scheme != "wss" || target.User != nil || target.Hostname() == "" || target.RawQuery != "" || target.Fragment != "" {
		return nil, false
	}
...
func (service *relayService) relayTunnel(request *http.Request, client *websocket.Conn, gameToken string) {
	...
	// Direct browser bots attach a completed Turnstile response as `cf:<token>`.
	// The browser sends the raw response to this relay in a WebSocket
	// subprotocol, so restore that wire prefix before dialing MooMoo. Without
	// it, MooMoo closes the relay's upstream socket before io-init and the HUD
	// can only report the generic "VPS relay needs attention" failure.
	if !strings.HasPrefix(gameToken, "cf:") {
		gameToken = "cf:" + gameToken
	}
	target.RawQuery = url.Values{"token": []string{gameToken}}.Encode()
