...
The browser sends a token-free MooMoo target in its relay-open JSON. Its fresh
game token travels only in the TLS WebSocket subprotocol header, is attached
to the upstream target inside the bridge, and is never written to a browser
URL, relay-open payload, or bridge log.
...
### Upgrade an existing relay

This release fixes browser WebSocket error `1006` by acknowledging the
per-connection Kitty token subprotocol during the upgrade. On the VPS, replace
the relay source with this directory, then rebuild and restart it:

```sh
cd /opt/kitty-vps-relay
go mod download
go build -trimpath -ldflags='-s -w' -o kitty-vps-relay .
sudo systemctl restart kitty-vps-relay
curl https://relay.example.com/health
```

Use the relay's actual public hostname in the last command. No environment
values need to change for this upgrade.
